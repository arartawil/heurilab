...
class CPO(_Base):
    def optimize(self):
        ...
        for t in range(self.max_iter):
            U = self.rng.random()  # threat level
            T_f = (1 - t / self.max_iter)  # time factor
            S = 2 * self.rng.random() - 1  # defense strategy factor
            ...
            for i in range(self.pop_size):
                ...
                if U < 0.3:
                    ...
                    new_X = X[i] + r1 * delta * np.sign(self.rng.random(self.dim) - 0.5) * T_f

                elif U < 0.6:
                    # Defense mode 2: Sound (exploitation — quill vibration)
                    r1 = self.rng.random(self.dim)
                    F = 2 * T_f * self.rng.standard_normal(self.dim)
