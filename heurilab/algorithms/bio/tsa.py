    ...
    def optimize(self):
        ...
        for t in range(self.max_iter):
            for i in range(self.pop_size):
                # Generate seeds
                n_seeds = self.rng.integers(1, max(2, self.pop_size // 5))
                ...
                for _ in range(n_seeds):
                    ...
                    for j in range(self.dim):
                        if self.rng.random() < ST:
                            # Use best tree
                            seed[j] = X[i, j] + self.rng.uniform(-1, 1) * (best[j] - X[k, j])
                        else:
                            # Use random tree
                            seed[j] = X[i, j] + self.rng.uniform(-1, 1) * (X[i, j] - X[k, j])
